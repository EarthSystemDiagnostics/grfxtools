# Code taken from function addnortharrow in package prettymapr v0.2.4:
# GPL-2
# Copyright (C) 2022 Dewey Dunnington

#' Plot north arrow
#'
#' Plot a north arrow (pointing directly "up") positioned based on current plot
#' extents.
#'
...
#' @source <https://CRAN.R-project.org/package=prettymapr>
#' @examples
#'
#' plot(1 : 5, 1 : 5, asp = 1)
#' AddNorthArrow()
                           ...
                           cols = c("white", "black"), text.col = "black")
{
  ...
  graphics::polygon(x - adj[1] * wd + arrow2.x * wd + wd/2, 
                    ...
                    lwd = lwd, col = cols[2])
  graphics::text(x - adj[1] * wd + wd/2, y - adj[2] * ht + 
                                         ht * 0, "N", cex = text.cex, adj = c(0.5, 0.5), col = text.col)
}
